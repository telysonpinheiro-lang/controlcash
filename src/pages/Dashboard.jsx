              ...
              <div className="alert-actions">
                {vencidasAll.slice(0, 3).map(c => (
                  <WaButton
                    ...
                    nome={c.cliente_nome}
                    telefone={c.telefone}
                    mensagem={`Olá ${c.cliente_nome?.split(' ')[0]}! Identificamos R$ ${Number(c.valor).toFixed(2)} em aberto (${c.referente}). Por favor, regularize o pagamento. — ControlCash`}
                    ...
                    <td>
                      {v.status === 'vencido' && (
                        <WaButton
                          ...
                          telefone={v.telefone}
                          mensagem={`Olá! Seu pagamento de R$ ${Number(v.valor).toFixed(2)} está vencido. Entre em contato para regularizar. — ControlCash`}
                          label="Cobrar"
                          userId={currentUser?.id}
                        />
                      )}
